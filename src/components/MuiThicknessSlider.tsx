...
// Custom styled slider for thickness controls
...
interface MuiThicknessSliderProps {
  value: number;
...
export default function MuiThicknessSlider({ 
  value, 
  onChange, 
  min = 0, 
  max = 200, 
  disabled = false,
  label = "How far the blur to go",
  showValue = true 
}: MuiThicknessSliderProps) {
  ...
  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(numValue) && numValue > 200) {
      return; // Don't allow values over 200
    }
    ...
    // Allow up to 3 digits, but cap at 200
    if (newInputValue.length <= 3) {
      if (!isNaN(numValue) && numValue >= min && numValue <= Math.min(max, 200)) {
        onChange(numValue);
      }
    }
  };
  ...
  const handleInputBlur = (event: React.FocusEvent<HTMLInputElement>) => {
    ...
    if (isNaN(newValue) || inputValue === '') {
      ...
      newValue = min;
    } else if (newValue > Math.min(max, 200)) {
      newValue = Math.min(max, 200); // Cap at 200
    }
    onChange(newValue);
  ...
  const handleIncrement = () => {
    const newValue = Math.min(Math.min(max, 200), value + 1);
    onChange(newValue);
  };
          ...
          <div className="relative flex items-center">
            ...
            <div className="absolute right-1 flex flex-col">
              <button
                ...
                disabled={disabled || value >= Math.min(max, 200)}
